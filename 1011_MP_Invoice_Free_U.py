...
def getCompanyInfo(company):
    temp=["Kari"]
    with open('1011_OCR_Regist.csv') as f:
        rows=csv.reader(f)
        for temp in rows:
            if ""==company:
                break
...
wb['Sheet'].cell(row=1, column=3,value='会社ｺｰﾄﾞ "01"')    #テスト  E0
...
wb1 = excel.Workbook()
wb1['Sheet'].cell(row=1, column=1,value='伝票No')
wb1['Sheet'].cell(row=1, column=2,value='仕訳日')
wb1['Sheet'].cell(row=1, column=3,value='取引先')
...
ii=0
company="Kari"
jj=101                                                      #UPLoadファイル用連番 ← 伝票№から変更 2020/11/30
...
for i in range(0, len(tifs)):
    try:
        ...
        if company=="" or tAmount=="":
            with open("1011_ErrorList_"+str(datetime.date.today().strftime('%Y%m%d'))+".txt","a+") as f:
        ...
        if int(税フラグ)==1:        
            ...
            tAmount=round(tAmount*1.1)
            消費税=round(tAmount/11)
        #チェックリストに記入
        wb1['Sheet'].cell(row=ii+2, column=1,value=jj)
        ...
        wb1.save('1011_Teiki_CheckList.xlsx')
        #upLoadするファイルに記入
        #読み取るたびに2列ずつ書き込む
        wb['Sheet'].cell(row=(2*ii)+2, column=1,value=jj)
        wb['Sheet'].cell(row=(2*ii)+3, column=1,value=jj)
        wb['Sheet'].cell(row=(2*ii)+2, column=2,value="RAK")
        wb['Sheet'].cell(row=(2*ii)+3, column=2,value="RAK")
        wb['Sheet'].cell(row=(2*ii)+2, column=3,value="01")      #テスト  E0
        wb['Sheet'].cell(row=(2*ii)+3, column=3,value="01")      #テスト  E0
...
wb = excel.load_workbook('1011_Upload_OCR.xlsx')
ws = wb["Sheet"]
wb.save('Upload_OCR_kari.xlsx')

wb2 = excel.load_workbook('Upload_OCR_kari.xlsx')
ws2 = wb2.worksheets[0]
with open('1011_Upload_OCR_経理.csv', 'w', newline="", encoding='cp932') as csvfile:
    writer = csv.writer(csvfile)
    for row in ws2.iter_rows(min_row=2):
        writer.writerow( [cell.value for cell in row] )

#定期支払チェックリスト.xlsx 読み合わせ用 列幅印刷設定
wb3 = excel.load_workbook('1011_Teiki_CheckList.xlsx')
ws3 = wb3["Sheet"]
ws3.delete_cols(9,3)
...
# 合計金額計算
sum1,sum2 = 0,0
Cell_tTtl=ws3.cell(row=rr+2,column=5).coordinate
Cell_tAmt=ws3.cell(row=rr+2,column=6).coordinate
Cell_tTax=ws3.cell(row=rr+2,column=7).coordinate
for row in ws3.iter_rows(min_col=6, min_row=2, max_col=6, max_row=rr):
    for cell in row:
        cell_value = cell.value
        sum1 += cell_value
ws3[Cell_tAmt] = sum1
ws3[Cell_tAmt].font = Font(bold=True)

for row in ws3.iter_rows(min_col=7, min_row=2, max_col=7, max_row=rr):
    for cell in row:
        cell_value = cell.value
        sum2 += cell_value
ws3[Cell_tTax] = sum2
ws3[Cell_tTax].font = Font(bold=True)

ws3[Cell_tTtl] = '              合      計'
ws3[Cell_tTtl].font = Font(bold=True)
# 金額カンマ編集
for rows in ws3.iter_rows(min_row=2,min_col=6,max_row=rr+2,max_col=7):
    for cell in rows:
        ws3[cell.coordinate].number_format = "#,##0" 
for rows in ws3.iter_rows(min_row=1,min_col=1,max_row=1,max_col=8):                                    
    for cell in rows:
        ws3[cell.coordinate].alignment = Alignment(horizontal = 'center', vertical = 'center')
...
wb3.save('1011_Teiki_CheckList_読み合わせ.xlsx')
