...
####################使い方###########################
#本スクリプト（または実行）ファイルと自動転記用のエクセル、企業別設定値
#が書いてあるcsvファイルを同一フォルダにおく。#
#上記場所に「undone」フォルダーを作成し、読み取りたい画像を格納しておく
#tif png pdfであれば対応してます
###################################################
...
def readImg(temp):
    ...
    #ocr用のコマンドを作成
    url = "https://x8g7u5g3e0.apigw.ntruss.com/custom/v1/113/9307b48c3263ae53e0eafba1f0caec356661f7d2b89a66b51593d7848f7c7ab4/infer"
    payload = "{\r\n    \"images\": [\r\n      {\r\n        \"format\": \"png\",\r\n        \"name\": \"All\",\r\n        \"data\": null,\r\n        \"url\": \""+ 'http://mikipulley.sakura.ne.jp/microsoft/azuru/ocr/img/files/'+temp+"\"\r\n      }\r\n    ],\r\n    \"lang\": \"ja\",\r\n    \"requestId\": \"string\",\r\n    \"resultType\": \"string\",\r\n    \"timestamp\": 1582166299,\r\n    \"version\": \"V1\"\r\n}"
    headers = {
            'Content-Type': 'application/json',
            'X-OCR-SECRET': 'VnRoTWdwU1ZFQmtkbFBldVp5bnZMaG1xZGdNR1FFZXM='
            }
    try:
        if "Nankai" in tifs[i]:
...
for i in range(0, len(tifs)):
    ...
    if s=="":
        s=0
    tAmount=int(s)
    print(str([tifs[i],"テンプレート名：MPHK_"+company+",invoiceNo:"+str(iNo),"金額:"+str(tAmount),code1,code2,name])+"\n")
    #iNoを正規表現で抽出する
    iNo=regmatch(iNo,reg)
    tAmount=tAmount/int(ritu)
    if company=="" or code1==0 or code2==0 or tAmount==0:#(conf1)<0.8 or (conf2)<0.8
        with open("undone\\file.txt","a+") as f:
            f.write(str([tifs[i],"テンプレート名：MPHK_"+company+",invoiceNo:"+str(iNo),"金額:"+str(tAmount),code1,code2,name])+"\n")
        continue
    #excel操作
